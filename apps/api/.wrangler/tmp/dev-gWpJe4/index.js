  ...
  "../../node_modules/@paralleldrive/cuid2/src/index.js"(exports, module) {
    ...
    var init = /* @__PURE__ */ __name(({
      ...
      return /* @__PURE__ */ __name(function cuid2() {
        ...
        const hashInput = `${time4 + salt + count3 + fingerprint}`;
...
// .wrangler/tmp/bundle-3JouPc/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-3JouPc/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
__export(src_exports, {
  ...
  deviceTokensRelations: () => deviceTokensRelations,
  highlights: () => highlights,
  highlightsRelations: () => highlightsRelations,
  libraryShelves: () => libraryShelves,
  libraryShelvesRelations: () => libraryShelvesRelations,
  otpTokens: () => otpTokens,
  readingGoals: () => readingGoals,
  readingGoalsRelations: () => readingGoalsRelations,
  readingProgress: () => readingProgress,
  readingProgressRelations: () => readingProgressRelations,
  readingStreaks: () => readingStreaks,
  readingStreaksRelations: () => readingStreaksRelations,
  refreshTokens: () => refreshTokens,
  refreshTokensRelations: () => refreshTokensRelations,
  sessions: () => sessions,
  sessionsRelations: () => sessionsRelations,
  shelfBooks: () => shelfBooks,
  shelfBooksRelations: () => shelfBooksRelations,
...
// ../../packages/db/src/schema.ts
var now = /* @__PURE__ */ __name(() => sql`(strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))`, "now");
...
var readingProgress = sqliteTable(
  "reading_progress",
  {
    id: text("id").primaryKey(),
    userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
...
var libraryShelves = sqliteTable("library_shelves", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
...
var refreshTokens = sqliteTable("refresh_tokens", {
  ...
  // Unix ms timestamp; null = still valid
});
var deviceTokens = sqliteTable("device_tokens", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
...
var readingGoals = sqliteTable("reading_goals", {
  id: text("id").primaryKey(),
  userId: text("user_id").notNull().unique().references(() => users.id, { onDelete: "cascade" }),
  dailyGoalMinutes: integer("daily_goal_minutes").notNull().default(5)
});
var readingStreaks = sqliteTable(
  "reading_streaks",
  {
    id: text("id").primaryKey(),
    userId: text("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
...
var otpTokens = sqliteTable("otp_tokens", {
  id: text("id").primaryKey(),
...
var booksRelations = relations(books, ({ one, many }) => ({
...
var highlightsRelations = relations(highlights, ({ one }) => ({
...
var libraryShelvesRelations = relations(libraryShelves, ({ one, many }) => ({
  user: one(users, { fields: [libraryShelves.userId], references: [users.id] }),
  shelfBooks: many(shelfBooks)
}));
var shelfBooksRelations = relations(shelfBooks, ({ one }) => ({
...
var deviceTokensRelations = relations(deviceTokens, ({ one }) => ({
  user: one(users, { fields: [deviceTokens.userId], references: [users.id] })
}));
var readingGoalsRelations = relations(readingGoals, ({ one }) => ({
  user: one(users, { fields: [readingGoals.userId], references: [users.id] })
}));
var readingStreaksRelations = relations(readingStreaks, ({ one }) => ({
...
function createDb(d1) {
  return drizzle(d1, { schema: src_exports });
}
...
async function signJwt(payload, secret) {
  ...
  return new SignJWT({ email: payload.email }).setProtectedHeader({ alg: "HS256" }).setSubject(payload.sub).setIssuedAt().setExpirationTime("15m").sign(key);
}
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
async function sendEmail(opts, env2) {
  const from = env2.MAIL_FROM ?? "noreply@bukoo.id";
  const payload = {
  ...
  if (env2.MAILCHANNELS_API_KEY) {
    headers["X-Auth-Api-Key"] = env2.MAILCHANNELS_API_KEY;
  }
...
async function sendOtpEmail(email, code, env2) {
  return sendEmail(
    {
      to: email,
...
var loginSchema = external_exports.object({
  ...
  deviceId: external_exports.string().default("unknown")
});
...
var socialAuthSchema = external_exports.object({
  provider: external_exports.enum(["google", "apple"]),
...
async function issueTokenPair(userId, email, deviceId, env2) {
  const db = createDb(env2.DB);
  const accessToken = await signJwt({ sub: userId, email }, env2.JWT_SECRET);
  const opaqueToken = (0, import_cuid2.createId)() + (0, import_cuid2.createId)();
...
__name(toUserPublic, "toUserPublic");
...
auth.post("/login", zValidator("json", loginSchema), async (c) => {
  ...
  const isValid2 = await verifyPassword(password, user.password);
  if (!isValid2) return c.json({ error: "Invalid email or password" }, 401);
...
auth.post("/refresh", zValidator("json", refreshSchema), async (c) => {
  ...
  if (record.revokedAt) {
    await db.update(refreshTokens).set({ revokedAt: Date.now() }).where(eq(refreshTokens.userId, record.userId));
    return c.json({ error: "Revoked token reuse detected. All sessions terminated." }, 401);
  }
  if (Date.now() > record.expiresAt) {
    return c.json({ error: "Refresh token expired" }, 401);
  }
  await db.update(refreshTokens).set({ revokedAt: Date.now() }).where(eq(refreshTokens.id, record.id));
...
auth.post("/social", zValidator("json", socialAuthSchema), async (c) => {
  ...
  if (provider === "google") {
    if (idToken.startsWith("mock-")) {
      ...
      const info3 = await res.json();
      if (info3.aud !== c.env.GOOGLE_CLIENT_ID) return c.json({ error: "Token audience mismatch" }, 401);
      email = info3.email;
      name = info3.name ?? email;
      avatar = info3.picture ?? null;
      providerAccountId = info3.sub;
      emailVerified = info3.email_verified === "true";
    }
  } else {
    if (idToken.startsWith("mock-")) {
  ...
  if (existingAccount) {
    ...
    if (!existingUser) {
      const newId = (0, import_cuid2.createId)();
      await db.insert(users).values({
        id: newId,
...
auth.post("/forgot-password", zValidator("json", forgotPasswordSchema), async (c) => {
  ...
  await db.insert(otpTokens).values({ id: (0, import_cuid2.createId)(), email: email.toLowerCase(), code, expiresAt });
  c.executionCtx.waitUntil(
    sendOtpEmail(email, code, c.env).catch(
      (err) => console.error("[forgot-password] Email send failed:", err)
    )
  );
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var TIER_ORDER = ["FREE", "PELAJAR", "PERSONAL", "PLUS", "FAMILY", "PREMIUM"];
function isBookAccessible(userSubscriptionTier, bookRequiredTier) {
...
__name(isBookAccessible, "isBookAccessible");
...
async function getUserTier(userId, db) {
...
function formatBook(book, userTier, progress, shelfSlug) {
  return {
    id: book.id,
    title: book.title,
    author: book.author,
...
booksRouter.get("/", zValidator("query", querySchema), async (c) => {
  ...
  if (genre) {
    ...
    const raw2 = await stmt.bind(...args).all();
    results = raw2.results ?? [];
  } else {
    results = await db.select().from(books).where(
      and(
        eq(books.isPublished, true),
        language ? eq(books.language, language.toUpperCase()) : void 0
      )
    ).orderBy(
      sort === "popular" ? desc(books.ratingCount) : sort === "newest" ? desc(books.createdAt) : sort === "rating" ? desc(books.ratingAverage) : books.id
...
booksRouter.get("/featured", async (c) => {
  ...
  if (cached) {
    const global2 = await cached.json();
    const continueReading2 = await db.select({ book: books }).from(readingProgress).innerJoin(books, eq(readingProgress.bookId, books.id)).where(and(eq(readingProgress.userId, userId), sql`${readingProgress.progressPercent} < 100`)).orderBy(desc(readingProgress.lastReadAt)).limit(10);
    return c.json({
      continue_reading: continueReading2.map(({ book: b }) => formatBook(b, userTier)),
      editors_choice: global2.editors_choice.map((b) => formatBook(b, userTier)),
      trending: global2.trending.map((b) => formatBook(b, userTier)),
      new_releases: global2.new_releases.map((b) => formatBook(b, userTier))
    });
  }
  const [editorsChoice, trending, newReleases, continueReading] = await Promise.all([
    ...
    db.select({ book: books }).from(readingProgress).innerJoin(books, eq(readingProgress.bookId, books.id)).where(and(eq(readingProgress.userId, userId), sql`${readingProgress.progressPercent} < 100`)).orderBy(desc(readingProgress.lastReadAt)).limit(10)
  ]);
...
booksRouter.get("/search", zValidator("query", external_exports.object({ q: external_exports.string().min(2) })), async (c) => {
  const userId = c.get("userId");
  const db = createDb(c.env.DB);
...
booksRouter.get("/:id", async (c) => {
  ...
  const [userTier, progress, shelfBook] = await Promise.all([
    ...
    db.select({ slug: libraryShelves.slug }).from(shelfBooks).innerJoin(libraryShelves, eq(shelfBooks.shelfId, libraryShelves.id)).where(and(eq(shelfBooks.bookId, bookId), eq(libraryShelves.userId, userId))).limit(1).then((r) => r[0] ?? null)
  ]);
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var reading = new Hono2();
reading.use("*", authMiddleware);
async function getUserTier2(userId, db) {
...
__name(getUserTier2, "getUserTier");
var updateProgressSchema = external_exports.object({
  bookId: external_exports.string().min(1),
  currentPage: external_exports.number().int().min(0).optional(),
  cfiPosition: external_exports.string().optional(),
  progressPercent: external_exports.number().min(0).max(100),
  reading_time_delta: external_exports.number().int().min(0).default(0)
...
reading.post("/progress", zValidator("json", updateProgressSchema), async (c) => {
  ...
  if (!book) return c.json({ error: "Book not found" }, 404);
  const userTier = await getUserTier2(userId, db);
  if (!isBookAccessible(userTier, book.subscriptionRequired)) {
...
reading.get("/recent", async (c) => {
  ...
  }).from(readingProgress).innerJoin(books, eq(readingProgress.bookId, books.id)).where(and(eq(readingProgress.userId, userId), sql`${readingProgress.progressPercent} < 100`)).orderBy(desc(readingProgress.lastReadAt)).limit(10);
...
reading.get("/highlights/:bookId", async (c) => {
  ...
  const userId = c.get("userId");
  const bookId = c.req.param("bookId");
  const result = await db.select().from(highlights).where(and(eq(highlights.userId, userId), eq(highlights.bookId, bookId))).orderBy(desc(highlights.createdAt));
  return c.json(result);
});
var highlightSchema = external_exports.object({
  cfiRange: external_exports.string().min(1),
...
reading.get("/bookmarks/:bookId", async (c) => {
  ...
  const userId = c.get("userId");
  const bookId = c.req.param("bookId");
  const result = await db.select().from(bookmarks).where(and(eq(bookmarks.userId, userId), eq(bookmarks.bookId, bookId))).orderBy(desc(bookmarks.createdAt));
  return c.json(result);
});
var bookmarkSchema = external_exports.object({
  cfi: external_exports.string().min(1),
  chapterTitle: external_exports.string().optional(),
...
goals.put("/", zValidator("json", external_exports.object({ dailyGoalMinutes: external_exports.number().int().min(1).max(1440) })), async (c) => {
  const db = createDb(c.env.DB);
  const userId = c.get("userId");
  ...
  if (existing) {
    ...
    await db.insert(readingGoals).values({ id: (0, import_cuid2.createId)(), userId, dailyGoalMinutes });
  }
...
goals.post(
  ...
  zValidator("json", external_exports.object({
    minutesRead: external_exports.number().int().min(1),
  ...
  async (c) => {
    ...
    const goal = await db.query.readingGoals.findFirst({ where: eq(readingGoals.userId, userId) }) ?? { dailyGoalMinutes: 5 };
    ...
    if (existing) {
      await db.update(readingStreaks).set({ minutesRead: total, goalMet }).where(and(eq(readingStreaks.userId, userId), eq(readingStreaks.date, today)));
    } else {
...
goals.get("/streaks", zValidator("query", external_exports.object({ days: external_exports.coerce.number().int().min(1).max(365).default(7) })), async (c) => {
  const db = createDb(c.env.DB);
  const userId = c.get("userId");
  ...
  const rows = await db.select().from(readingStreaks).where(
    and(
...
goals.get("/streak/current", async (c) => {
  const db = createDb(c.env.DB);
  const userId = c.get("userId");
  const allStreaks = await db.select().from(readingStreaks).where(eq(readingStreaks.userId, userId)).orderBy(desc(readingStreaks.date));
  let currentStreak = 0;
...
goals.get("/books-this-year", async (c) => {
  ...
  const startOfYear = `${year2}-01-01T00:00:00.000Z`;
  const endOfYear = `${year2}-12-31T23:59:59.999Z`;
  const result = await db.selectDistinct({ bookId: readingProgress.bookId }).from(readingProgress).where(
    and(
...
var summarySchema = external_exports.object({
  chapterText: external_exports.string().min(10).max(1e4),
  bookTitle: external_exports.string().optional(),
  chapterTitle: external_exports.string().optional()
});
...
Buatkan rangkuman singkat (3\u20135 poin utama) dalam Bahasa Indonesia, tulis dalam format poin-poin jelas yang membantu pembaca memahami isi bab ini.`
  ...
  const response = await c.env.AI.run("@cf/meta/llama-3-8b-instruct", {
    messages: [
      {
        role: "system",
        content: "Kamu adalah asisten membaca yang membantu pengguna memahami isi buku. Berikan rangkuman yang jelas, informatif, dan mudah dipahami dalam Bahasa Indonesia."
      },
...
app.route("/v1/auth", auth_default);
app.route("/v1/books", books_default);
app.route("/v1/reading", reading_default);
app.route("/v1/goals", goals_default);
app.route("/v1/users", users_default);
