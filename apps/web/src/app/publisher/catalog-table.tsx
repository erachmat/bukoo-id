  ...
  return (
    <div className="pds-panel">
          ...
          <tbody>
            {books.length === 0 ? (
              <tr><td colSpan={8} style={{ padding: '48px 16px', textAlign: 'center', color: 'var(--pds-muted)', fontSize: '12px' }}>
                {books.length === 0 ? <>Belum ada buku terdaftar. <Link href="/publisher/books/new" style={{ color: 'var(--pds-teal)', fontWeight: 600, textDecoration: 'none' }}>Upload buku pertama Anda →</Link></> : 'Tidak ada buku yang cocok dengan filter saat ini.'}
              </td></tr>
      ...
      <div className="pds-catalog-pagination">
        ...
        <div><button type="button" disabled={paginated.page === 1} onClick={() => setPage(paginated.page - 1)}>Sebelumnya</button><span>Halaman {paginated.page} / {paginated.totalPages}</span><button type="button" disabled={paginated.page === paginated.totalPages} onClick={() => setPage(paginated.page + 1)}>Berikutnya</button></div>
      </div>
    </div>
  );
}