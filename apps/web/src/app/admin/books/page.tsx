  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 800, color: 'var(--ad-text)', margin: 0 }}>Kelola Buku</h1>
          <p style={{ color: 'var(--ad-dim)', marginTop: 6, fontSize: 14 }}>
      ...
      <div style={{ background: 'var(--ad-panel)', borderRadius: 16, border: '1px solid var(--ad-border)', overflow: 'hidden', boxShadow: '0 4px 18px rgba(0,0,0,0.3)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {books.length === 0 && (
              <tr>
                <td colSpan={7} style={{ padding: '48px 16px', textAlign: 'center', color: 'var(--ad-muted)', fontSize: 14 }}>
            ...
            {books.map((book: typeof books[number], i: number) => (
              <tr key={book.id} style={{ borderBottom: i < books.length - 1 ? '1px solid var(--ad-border)' : 'none' }}>
                <td style={{ padding: '14px 16px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    {book.coverKey ? (
                      <img src={getCoverUrl(book.coverKey)} alt={book.title} style={{ width: 36, height: 52, objectFit: 'cover', borderRadius: 4, flexShrink: 0, border: '1px solid var(--ad-border)' }} />
                    ) : (
                      <div style={{ width: 36, height: 52, background: 'var(--ad-panel-raised)', borderRadius: 4, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: 'var(--ad-muted)' }}>?</div>
                    )}
                    <div>
                      <div style={{ fontWeight: 700, color: 'var(--ad-text)', fontSize: 14 }}>{book.title}</div>
                      <div style={{ fontSize: 12, color: 'var(--ad-muted)', marginTop: 2 }}>{book.author}</div>
                    </div>
                  </div>
                </td>
                <td style={{ padding: '14px 16px' }}><FeaturedToggleButton bookId={book.id} featured={book.featured} disabled={!book.isPublished} /></td>
                <td style={{ padding: '14px 16px' }}>
                  <span style={{ background: 'var(--ad-panel-raised)', color: 'var(--ad-dim)', fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 6 }}>
                    {book.genre[0] ?? '—'}
                  </span>
                </td>
                <td style={{ padding: '14px 16px', fontSize: 13, color: 'var(--ad-dim)' }}>{book.language}</td>
                <td style={{ padding: '14px 16px' }}>
                  {book.subscriptionRequired !== 'FREE' ? (
                    <span style={{ background: 'rgba(245,158,11,0.1)', color: 'var(--ad-amber-lt)', fontSize: 11, fontWeight: 800, padding: '3px 8px', borderRadius: 6 }}>{book.subscriptionRequired}</span>
                  ) : (
                    <span style={{ background: 'rgba(0,201,167,0.1)', color: '#00c9a7', fontSize: 11, fontWeight: 800, padding: '3px 8px', borderRadius: 6 }}>GRATIS</span>
                  )}
                </td>
                <td style={{ padding: '14px 16px' }}>
                  {book.isPublished ? (
