...
- `apps/web` — Next.js web app (NextAuth v5, Drizzle ORM), **deployed to Cloudflare Workers** → https://bukoo.id (migrated from Vercel 2026-08-16; Vercel decommissioned). DB is **Cloudflare D1** `bukoo-db` via the `DB` binding (NOT Neon/Postgres).
  - Web auth (2026-08-20): one worker serves BOTH `bukoo.id` and `publisher.bukoo.id` (host routing in `src/middleware.ts`). Shared customer auth at `src/app/(auth)/` (`login`/`register`/`forgot-password`) + dedicated branded pages `src/app/publisher/login` & `src/app/publisher/register`. Middleware maps publisher-host `/login`→`/publisher/login`; `/publisher/submit` requires `role === "PUBLISHER"` (middleware-gated). NOTE: `/publisher/dashboard` is NOT middleware-gated — it is a public showcase page (`src/app/publisher/dashboard/page.tsx`) that renders `<DashboardClient>` only when `role === "PUBLISHER"`, else a public `<PublisherDashboardShowcase />` (corrected 2026-08-29; AGENTS.md previously claimed it was gated). Other publisher pages self-guard via `auth()`/`getPublisherUser()`. Self-serve publisher signup (`signUpPublisher` in `(auth)/actions.ts`) creates `role: 'PUBLISHER'` immediately. **All sign-out must use the server action `signOut()` from `(auth)/actions.ts`** (client `next-auth/react` signOut was unreliable at clearing the JWT cookie on Workers). Error keys map through `src/app/(auth)/errors.ts`; callbackUrl is sanitized by `safeCallbackUrl()` in `src/lib/auth-helpers.ts`. `allowDangerousEmailAccountLinking: true` is intentional (auto-link Google↔email accounts).
  - Web auth hardening (2026-08-29): password reset is a **two-step OTP flow** (`requestPasswordReset` + `verifyPasswordReset` in `(auth)/actions.ts`) — the old single-step `resetPassword` (unauthenticated account takeover) is REMOVED. OTPs live in the shared `otp_tokens` table (15 min expiry, 6-digit code), emailed via MailChannels (`src/lib/mail.ts`). **All auth actions are rate-limited via D1** (`src/lib/rate-limit.ts`, table `auth_attempts`): login 5 fails/15min per email → 15min lock; 10/15min per IP → 1h lock; register 5/h per IP; OTP request 3/15min per email; OTP verify 5/15min per email. Increments happen in the server actions; `authorize()` in `src/lib/auth.ts` only *reads* the lock (fail-open). MailChannels secrets: `MAILCHANNELS_API_KEY`, `MAIL_FROM` (both optional — mail falls back gracefully).
...
- **Web → Cloudflare Workers** (`apps/web`): build + deploy from `apps/web`:
  ...
  - Secrets on the worker (set via `wrangler secret put`): `AUTH_SECRET`, `AUTH_GOOGLE_ID`,
    `AUTH_GOOGLE_SECRET`, `MAILCHANNELS_API_KEY`, `MAIL_FROM` (the last two optional — web
    password-reset OTP email, added 2026-08-29; both also used by `apps/api`).
