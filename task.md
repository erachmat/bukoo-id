...
- `[x]` 2. **The `/_next/image` optimizer was a silent passthrough.** `Hero.tsx` used `<Image priority sizes="100vw">`, generating a proper 640→3840px srcSet — but every width returned the identical 9,701,680 bytes (verified byte-identical by sha256 against the raw asset). Cause is in OpenNext's own source, `@opennextjs/cloudflare/dist/cli/templates/images.js:94`: *"Image optimization is disabled and the original image is returned if `env.IMAGES` is undefined."* **Neither `wrangler.jsonc` nor `wrangler.prod.jsonc` declares an `images` binding.** So the responsive srcSet was inert and every visitor downloaded 9.25 MB. The CSS `url()` bypassed the optimizer entirely.
- `[x]` 3. Fix: pre-encoded variants at 1280/1920/2560/3840 in both WebP (q80) and JPEG (q82) — `hero01-{w}.{webp,jpg}`. `Hero.tsx` switched from `next/image` to a plain `<picture>` with `type="image/webp"` + a real `srcSet` (documented in a code comment, since `next/image` here would ship the original to every viewport). The CSS hero switched to `image-set()` with a plain `url()` fallback. Alpha channel was verified fully opaque (255..255) before dropping to JPEG.
- `[x]` 4. Moved the `fill` prop's positioning into CSS — `next/image`'s `fill` supplied `position:absolute; inset:0`, so `.homepage-hero-image` now sets those explicitly.
- `[x]` 5. Deleted the 9.25 MB `hero01.png` (still in git history) — it would otherwise keep shipping in every deploy. `public/homepage-assets` 12.0 MB → 2.8 MB.
- `[x]` 6. Verified in a real browser: at 1274px the browser requests **one** file — `hero01-1280.webp`, 31 KB. Variant selection correct at 1920 (→1920.webp), 2560 (→2560.webp), and 1440@2x (→3840.webp). Publisher CSS hero resolves to `hero01-1280.webp`; zero requests for `hero01.png`; old path now 404s. Zero horizontal overflow at 1440/390 on both surfaces; screenshot-verified the hero still renders correctly. **9.25 MB → 31 KB on a typical desktop (~99.7%)**, 73 KB worst case at 2560.
- `[ ]` 7. Flagged, not fixed: `Group01/02/03.png` (575–676 KB each) are served raw and unoptimized on the reader homepage — same class of problem, separate change. `publisher-hero.jpg`/`publisher-hero@1280.jpg` are dead (zero `src/` references).
- `[ ]` 8. Systemic option, not taken: adding an `images` binding to both wrangler configs would make `/_next/image` genuinely resize/convert for *every* image. It's a **paid** Cloudflare feature — verify entitlement and cost before adopting.

# Publisher Homepage URL — `/publisher/daftar` Leak Fixed — 2026-09-19

- `[x]` 1. Cause: `src/middleware.ts` handled the bare publisher domain with `NextResponse.redirect("/publisher/daftar")`, so `publisher.bukoo.id/` advertised an internal path in the address bar on a domain that is already publisher-scoped. A second `redirect()` in `src/app/publisher/page.tsx` did the same for `/publisher`.
- `[x]` 2. Changed the middleware branch to `NextResponse.rewrite("/publisher/daftar")` — serves the landing page while keeping the URL as `/`. The route path is forced by architecture: one worker serves both `bukoo.id` and `publisher.bukoo.id`, so `/` is already the reader homepage. The file already used `rewrite` for the `/penerbit-*.html` aliases, so the pattern was established; it just hadn't been applied to `/`.
- `[x]` 3. Added `alternates.canonical = "https://publisher.bukoo.id/"` to the landing page metadata. Without it the same page answers on four URLs (`publisher.bukoo.id/`, `publisher.bukoo.id/publisher/daftar`, `bukoo.id/publisher/daftar`, and the `.html` alias), all competing. The canonical names the one real URL.
- `[x]` 4. Fixed the links that re-exposed the path: `LandingFooter` "Daftar Penerbit" → `/` (it pointed at `/publisher/daftar`, i.e. the self-link would have undone the fix on click), `LandingNav` logo → `/`, `LandingFooter` "Homepage Pembaca" → `https://bukoo.id/` (a bare `/` is a self-link on the publisher host), and `(marketing)/MarketingFooter` publisher links → `https://publisher.bukoo.id/`.
- `[x]` 5. Verified by HTTP with a spoofed `Host` header: `publisher.bukoo.id/` → `200` + `x-middleware-rewrite: /publisher/daftar` (no `Location`, so the browser cannot change the URL bar — confirmed no client-side `router.replace/push` exists either); `bukoo.id/` unchanged `200` reader homepage; `/penerbit-daftar.html` alias still rewrites; `/daftar` still 307s. The rendered publisher page now contains **zero** `href="/publisher/daftar"` links and carries the canonical tag.
- `[x]` 6. Auth path unaffected: `shouldBouncePublisherFromLanding` already matches `pathname === "/"` (unit-covered, `publisher-landing-guard.test.ts:18`), and it runs *before* the rewrite branch, so signed-in publishers still go to `/publisher/dashboard`. Sign-out (`?logout=1`) still bypasses via its absolute `/publisher/daftar` pathname. Gates: lint 0 errors (28 warnings), tsc exit 0, tests 86/86.
- `[ ]` 7. Flagged, not fixed (pre-existing, out of scope): `bukoo.id/publisher/daftar` still returns `200`, so the publisher landing page is also served on the reader host. The canonical tag mitigates the SEO duplication; a proper fix would bounce `/publisher/*` on the reader host to the publisher domain.
...
- `[x]` 6. **P2 — gold consolidated 4 → 1.** Measured both Figma frames: the only gold-family *fill* is `#B58418` (77% of gold pixels in the publisher frame, 78% in the reader frame). `#C9952A` = 4px / 1px, `#D4A33E` = 0px, `#D39A0A` = 0px — all antialiasing or production-only inventions. Collapsed `--bl-gold-2/3/4` (deleted) and `--amber` into `#B58418`; `.nav.minimal .btn-cta` moved off `#D39A0A` (white on it was 2.50:1, below AA). Retained `--bl-gold-hover #A07015` (Figma's darker gold, 1651px) and `--amber-lt #F0D080` (real node-tree token, also in mobile `COLORS.ts`). ~40 derived `rgba(201,149,42,…)` tints re-based to `rgba(181,132,24,…)`.
- `[x]` 7. **P2 — `Masuk` terracotta corrected.** The earlier claim that `#b74822` "is not in the Figma palette" was **wrong**: the rendered publisher header's `Masuk` button is a solid `#B64717` fill (3284px). Corrected the value; white on it is 5.38:1 (AA).
- `[x]` 8. **P2 — button radius corrected 8/10/16 → 12px.** The earlier "Figma ≈7px, production already correct" claim was also **wrong**. Three independent methods (corner-deficit area, analytic fit, synthetic-profile match) converge on **12.5–13px** for all three header/hero buttons in both frames. Fixed `.bl-btn-login` 8→12, `.bl-btn-gold` 8→12, `.bl-btn-gold.bl-lg` 10→12, `.nav.minimal .btn-ghost/.btn-cta` 16→12.
- `[x]` 9. Reference cleanup: `references/figma-design.md` — replaced the superseded "≈7px" radius prose, corrected the `--amber`/`--border` token listing, relabelled `#B64717` from "danger" to the `Masuk` fill, and added a "Gold — one value, not four" section recording the measured pixel counts.
- `[x]` 11. **Follow-up sweep (same day, commit `e5493d2` successor).** Verifying against the *served CSS chunks* rather than the source caught three things a source-only review missed: (a) `--amber: #C9952A` in `publisher/publisher.css` — a 5th live gold feeding **82** derived `rgba()` tints plus `--pds-amber`; (b) the reader nav CTA's hover state, `#E0AA19` background + `rgba(211,154,10,…)` shadow (both 0px in the frames → now `#A07015`); (c) a hardcoded `#C9952A` on the publisher auth logo. All re-based to `#B58418`/`#A07015`. Confirmed both publisher routes compile `--amber:#b58418` + `--pds-amber:#B58418` with zero old values.
- `[x]` 12. Scope note: 5 further files still carry `rgba(201,149,42,…)` tints — `admin/admin.css` (`--ad-amber`), `(app)/layout.tsx`, `(auth)/layout.tsx`, `publisher/dashboard/dashboard-client.tsx`, `(marketing)/penerbit/page.tsx`, `components/marketing/{Testimonials,BookRow}.tsx`. **None render on `publisher.bukoo.id/` or `bukoo.id/`** (verified by checking each homepage's component imports), so they are out of scope for this alignment and were left untouched. Worth a separate sweep if the admin/auth surfaces are meant to share the token.
...
- `[x]` Homepage `Masuk`/`Coba Gratis` controls are right-aligned at desktop and mobile widths.
- `[x]` Navbar hides with an animated upward transform after downward scrolling past 80px and reappears when scrolling upward; mobile drawer state keeps the nav visible.
