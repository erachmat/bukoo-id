    ...
    <form onSubmit={handleSubmit}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 24, alignItems: 'flex-start' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #E8ECF0', padding: 28, boxShadow: '0 2px 12px rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div>
            <label style={labelStyle}>Tipe Akses *</label>
            <div style={{ display: 'flex', gap: 12 }}>
              {[{ value: 'false', label: 'Gratis', color: '#00C9A7' }, { value: 'true', label: 'Premium', color: '#F59E0B' }].map(opt => (
                <label key={opt.value} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', padding: '10px 16px', borderRadius: 10, border: '1px solid #E8ECF0', flex: 1, fontSize: 14, fontWeight: 600, color: '#3D4A5C' }}>
                  <input type="radio" name="isPremium" value={opt.value} defaultChecked={String(defaultValues.isPremium ?? true) === opt.value} style={{ accentColor: opt.color }} />
                  {opt.label}
                </label>
              ))}
            </div>
